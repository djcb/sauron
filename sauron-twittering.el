...
;;
;; Copyright (C) 2011 Dirk-Jan C. Binnema, Joel McCracken
  ...
  (if (and (boundp 'twittering-mode-version)
      ...
      (progn
        (when sauron-twittering-running
          (error "sauron-twittering is already running. Call sauron-twittering-stop first."))
        
        (add-hook 'twittering-new-tweets-hook 'sauron-twittering-new-tweets-func)
        (setq sauron-twittering-running t))
    (message  "No twittering, so sauron-twittering could not start")))
