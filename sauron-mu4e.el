...
(defun sauron-mu4e-uread-messages (mails)
  ...
  (dolist (mail mails)
    (let ((log-message (sauron-mu4e-formatter mail))
...
(defun sauron-mu4e-formatter (mail)
  ...
  (let
      ((subject (plist-get mail :subject))
       (type (if (or
		  (plist-get mail :references)
		  (plist-get mail :in-reply-to)) "Reply" "Mail"))
       (from (car (plist-get mail :from))))
    (let
       ((name (car from))
       (email (cdr from)))
      (format "%s from %s: %s" type name subject))))
...
(defun sauron-mu4e-make-link (mail)
  ...
  (lexical-let
    ...
    (lambda () (mu4e-view-message-with-msgid message-id))))
